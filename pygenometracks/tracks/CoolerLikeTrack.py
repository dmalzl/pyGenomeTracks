...
class CoolerLikeTrack(GenomeTrack):
    SUPPORTED_ENDINGS = []
    TRACK_TYPE = None
    OPTIONS_TXT = """
...
# rasterize = false
    ...
    BOOLEAN_PROPERTIES = ['show_masked_bins', 'rasterize', 'divisive_weights']
    ...
    def __init__(self, *args, **kwargs):
        super(CoolerLikeTrack, self).__init__(*args, **kwargs)
        log.debug(f'FILE {self.properties}')

    def set_properties_defaults(self):
        super(CoolerLikeTrack, self).set_properties_defaults()
        ...
        self.clr = cooler.Cooler(self.properties['file'])
    ...
    def check_before_plotting(self, chrom_region, region_start, region_end, suffix='', _pass = True):
        # we simply pass here because this is all hicMatrix related code and I did not want to mess with it yet
        if _pass:
